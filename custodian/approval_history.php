<?php
/**
 * Custodian Approval History Page
 * Shows all requests approved/rejected by this custodian
 */

require_once dirname(__DIR__) . '/config.php';

// Require login and custodian role
requireLogin();
if (!hasRole('custodian')) {
...
try {
    // Total approved by this custodian
    $stmt = $pdo->prepare("
        SELECT COUNT(*) as count
        FROM asset_requests
        WHERE custodian_reviewed_by = ?
        AND status IN ('approved', 'released', 'returned', 'office_review', 'department_review')
    ");
    $stmt->execute([$userId]);
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE custodian_reviewed_by = ?
        AND status IN ('approved', 'released', 'returned', 'office_review', 'department_review')
        AND YEAR(custodian_reviewed_at) = YEAR(CURDATE())
        AND MONTH(custodian_reviewed_at) = MONTH(CURDATE())
    ");
    $stmt->execute([$userId]);
...
<head>
    ...
    <title>Approval History - Custodian Dashboard</title>
...
<body class="bg-gray-100">
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mt-1">
                                ...
                                Campus: <strong><?= htmlspecialchars($user['campus_name'] ?? 'N/A') ?></strong>
                            </p>
                        </div>
    ...
    class HistoryManager {
        ...
        async loadHistory() {
            ...
            try {
                const response = await fetch('/AMS-REQ/api/requests.php?action=get_custodian_approval_history');
        ...
        applyFilters() {
            ...
            this.filteredHistory = this.history.filter(request => {
                ...
                if (dateRangeFilter !== 'all') {
                    const reviewedDate = new Date(request.custodian_reviewed_at);
                    const now = new Date();

                    switch (dateRangeFilter) {
                        case 'today':
                            if (reviewedDate.toDateString() !== now.toDateString()) return false;
                            break;
                        case 'week':
                            const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
                            if (reviewedDate < weekAgo) return false;
                            break;
                        case 'month':
                            if (reviewedDate.getMonth() !== now.getMonth() || reviewedDate.getFullYear() !== now.getFullYear()) return false;
                            break;
                        case 'year':
                            if (reviewedDate.getFullYear() !== now.getFullYear()) return false;
                            break;
                    }
                }
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition-all">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            ${request.custodian_review_notes ? `
                                <div class="bg-gray-50 rounded p-2 mt-2 text-sm">
                                    ...
                                    <p class="text-gray-700 mt-1">${this.escapeHtml(request.custodian_review_notes)}</p>
                                </div>
                            ` : ''}
        ...
        getStatusBadge(status) {
            const badges = {
                ...
                'office_review': '<span class="badge badge-office_review">Office Review</span>',
                'department_review': '<span class="badge badge-department_review">Department Review</span>'
            };
        ...
        async viewDetails(requestId) {
            try {
                ...
                if (data.success) {
                    ...
                    const reviewedDate = new Date(request.custodian_reviewed_at).toLocaleString();
                        ...
                        html: `
                            <div class="text-left space-y-3">
                                ...
                                <div class="border-b pb-2">
                                    ...
                                    <p class="font-semibold">${reviewedDate}</p>
                                </div>
                                <div class="border-b pb-2">
                                ...
                                ${request.custodian_review_notes ? `
                                    <div class="border-b pb-2">
                                        ...
                                        <p class="font-semibold">${this.escapeHtml(request.custodian_review_notes)}</p>
                                    </div>
                                ` : ''}
                                ${request.status === 'rejected' && request.rejection_reason ? `
