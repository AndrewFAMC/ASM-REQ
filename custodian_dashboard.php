    ...
    try {
        switch ($_POST['action']) {
            case 'add_asset':
                ...
                echo json_encode(['success' => true, 'message' => 'Asset added successfully!', 'data' => $result]);
                break;
            ...
            case 'assign_and_generate_tag':
                ...
                // 3. Update asset status to 'Active' if quantity becomes 0
                $newQuantity = $currentAsset['quantity'] - $quantity;
                if ($newQuantity == 0) {
                    executeQuery($pdo, "UPDATE assets SET status = 'Inactive' WHERE id = ?", [$assetId]);
                }
            ...
            case 'assign_asset_to_office':
                ...
                // 2. Update asset status to Inactive (pending office verification)
                executeQuery($pdo, "UPDATE assets SET status = 'Inactive' WHERE id = ?", [$assetId]);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadOfficeList() {
        ...
        try {
            const res = await apiRequest('custodian_dashboard.php', 'get_office_list');
            if (res && res.success) {
    ...
    async function loadCategoriesIntoSelect() {
        ...
        try {
            const res = await apiRequest('custodian_dashboard.php', 'get_categories');
            if (res && res.success) {
    ...
    async function loadAllAssets() {
        ...
        try {
            const res = await apiRequest('custodian_dashboard.php', 'get_all_assets');
    ...
    async function loadAssetsForOffice() {
        ...
        try {
            const res = await apiRequest('custodian_dashboard.php', 'get_assets_for_office', {
    ...
    window.viewAssetDetails = async function() {
        const res = await apiRequest('custodian_dashboard.php', 'get_asset_details', { asset_id: selectedAsset.id });
        if (res.success) {
    ...
    document.getElementById('stockForm').addEventListener('submit', async function(e) {
        ...
        const res = await apiRequest('custodian_dashboard.php', 'update_stock', {
    ...
    window.openAssignOfficeModal = async function() {
        ...
        closeActionsModal();        const res = await apiRequest('custodian_dashboard.php', 'get_offices');
        if (res.success) {
    ...
    document.getElementById('generateTagForm').addEventListener('submit', async function(e) {
        ...
        const res = await apiRequest('custodian_dashboard.php', 'assign_and_generate_tag', data);
        if (res.success) {
    ...
    document.getElementById('inventoryTagForm').addEventListener('submit', async function(e) {
        ...
        const res = await apiRequest('custodian_dashboard.php', 'assign_asset_to_office', data);
    ...
    async function openPrintableTagModal(tagId) {
        const res = await apiRequest('custodian_dashboard.php', 'get_tag_details_for_print', { tag_id: tagId });
        if (res.success) {
            const tag = res.data;
            ...
            document.getElementById('pt_article').innerText = tag.article;
            ...
            document.getElementById('pt_remarks').innerText = tag.remarks;
    ...
    addAssetForm.addEventListener('submit', async function(e) {
        ...
        // The createAsset function in asset_actions.php will handle setting the status to 'Inactive'
        const res = await apiRequest('custodian_dashboard.php', 'add_asset', data);

        if (res.success) {
            Swal.fire('Success!', 'New asset has been added with status "Inactive".', 'success');
            closeAddAssetModal();
