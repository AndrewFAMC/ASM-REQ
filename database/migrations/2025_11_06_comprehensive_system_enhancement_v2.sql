...
-- First, check if constraints exist and drop them if needed
SET @exist := (SELECT COUNT(*) FROM information_schema.table_constraints
    WHERE constraint_schema = DATABASE() AND table_name = 'asset_requests'
    AND constraint_name = 'fk_request_custodian_reviewer');
SET @sqlstmt := IF(@exist > 0, 'ALTER TABLE asset_requests DROP FOREIGN KEY fk_request_custodian_reviewer', 'SELECT ''Constraint does not exist''');
PREPARE stmt FROM @sqlstmt;
EXECUTE stmt;

SET @exist := (SELECT COUNT(*) FROM information_schema.table_constraints
    WHERE constraint_schema = DATABASE() AND table_name = 'asset_requests'
    AND constraint_name = 'fk_request_dept_approver');
SET @sqlstmt := IF(@exist > 0, 'ALTER TABLE asset_requests DROP FOREIGN KEY fk_request_dept_approver', 'SELECT ''Constraint does not exist''');
PREPARE stmt FROM @sqlstmt;
EXECUTE stmt;

SET @exist := (SELECT COUNT(*) FROM information_schema.table_constraints
    WHERE constraint_schema = DATABASE() AND table_name = 'asset_requests'
    AND constraint_name = 'fk_request_final_approver');
SET @sqlstmt := IF(@exist > 0, 'ALTER TABLE asset_requests DROP FOREIGN KEY fk_request_final_approver', 'SELECT ''Constraint does not exist''');
PREPARE stmt FROM @sqlstmt;
EXECUTE stmt;
...
-- ============================================================================
-- 15. CREATE OR UPDATE SYSTEM SETTINGS TABLE (FOR CONFIGURATION)
-- ============================================================================

-- Drop existing system_settings if it exists with wrong structure
...
-- Now insert default system settings (table exists for sure)
...
GROUP BY o.id, o.name, c.campus_name;
