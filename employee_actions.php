...
function returnAsset($pdo, $data) {
    ...
    try {
        ...
        // Instead of returning to the general 'assets' pool, return it to the 'inventory_tags' borrowable quantity
        // This assumes the asset was borrowed from an office's borrowable pool.
        // We find the inventory tag this asset belongs to. This logic might need refinement
        // if assets can be assigned from multiple sources.
        executeQuery($pdo, "UPDATE inventory_tags SET borrowable_quantity = borrowable_quantity + ? WHERE asset_id = ? AND office_id = (SELECT office_id FROM users WHERE id = ?)", [$returnQuantity, $assetId, $userId]);

        $description = "{$returnQuantity} unit(s) of asset returned by employee. " . ($remainingQuantity > 0 ? "{$remainingQuantity} remaining." : "Fully returned.") . ($returnNotes ? " Notes: {$returnNotes}" : "");
...
function getEmployeeStats($pdo, $campusId = null) {
    $campusId = $_SESSION['campus_id'] ?? null;
...
function getMyActivities($pdo, $campusId = null, $limit = 10) {
    ...
    $sql = "SELECT al.*, a.asset_name, cam.campus_name 
            ...
            WHERE al.performed_by LIKE '%Employee%' OR al.action IN ('REQUESTED', 'RETURNED')";
...
function createAsset($pdo, $data) {
    try {
        ...
        if ($existingAssignment) {
            ...
            logActivity($pdo, $existingAssignment['asset_id'], 'STOCK_IN', "Employee added {$newQuantity} more unit(s) to existing asset.");
            $pdo->commit();
        ...
        // Auto-assign to current employee member
        ...
        $stmt->execute([
            ...
            $data['value'] ?? 0,
            $data['status'] ?? 'Active', // Explicitly set to Active for employee-created assets
            $assignedTo,
            $assignedEmail,
            $assignmentDate,
            $createdBy,
        ...
        // Log activity
        logActivity($pdo, $assetId, 'CREATED', "Asset '{$data['asset_name']}' created by employee member {$assignedTo}");
...
function recordBorrowing($pdo, $data) {
    ...
    if (!empty($asset['assigned_email']) && $asset['assigned_email'] !== $_SESSION['email']) {
        throw new Exception("Asset is assigned to another employee member and cannot be borrowed");
    }
...
function borrowAssetForSelf($pdo, $assetId) {
    ...
    logActivity($pdo, $assetId, 'ASSIGNED', "Asset '{$asset['asset_name']}' was borrowed by employee member {$user['full_name']}.");

    return ['asset_id' => $assetId, 'assigned_to' => $user['full_name'], 'assignment_date' => date('Y-m-d')];
}

function getLatestEmployeeActivityTimestamp($pdo) {
    $userEmail = $_SESSION['email'] ?? null;
    $campusId = $_SESSION['campus_id'] ?? null;
    ...
    // Query for the latest activity timestamp relevant to the employee member.
    // This includes:
...
function requestSpecificAsset($pdo, $assetId, $quantity = 1, $purpose = '', $expectedReturnDate = null) {
    ...
    logActivity($pdo, $assetId, 'REQUESTED', "Asset requested by employee member {$user['full_name']}");
